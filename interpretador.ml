...
let rec tipo_str (tp:tipo) : string =
  ...
  | TyEither (t1,t2)-> "(" ^ (tipo_str t1) ^ " either " ^ (tipo_str t2) ^ ")"
...
let rec expr_str (e:expr) : string  =
  ...
  | If (e1,e2,e3) -> "(if " ^ (expr_str e1) ^ " then " ^ (expr_str e2) ^ " else " ^ (expr_str e3) ^ " )"
  ...
  | MatchNil (e1,e2,e3) -> "(match " ^ (expr_str e1) ^ " with [] -> " ^ (expr_str e2) ^ " | x::xs => " ^ (expr_str e3) ^ ")"
  | Nothing -> "Nothing"
  | Just e1 -> "(Just " ^ (expr_str e1) ^ ")"
  | MatchNothing (e1,e2,e3) -> "(match " ^ (expr_str e1) ^ " with Nothing -> " ^ (expr_str e2) ^ " | Just x => " ^ (expr_str e3) ^ ")"
  | Left e1 -> "(Left " ^ (expr_str e1) ^ ")"
  | Right e1 -> "(Right " ^ (expr_str e1) ^ ")"
  | MatchLeft (e1,e2,e3) -> "(match " ^ (expr_str e1) ^ " with Left x => " ^ (expr_str e2) ^ " | Right y => " ^ (expr_str e3) ^ ")"
...
let rec appsubs (s:subst) (tp:tipo) : tipo =
  ...
  | TyVar  x        -> (match lookup s x with
      ...
      (* ======= TRABALHO ======= *)
  | TyList x         -> TyList (appsubs s x)
  | TyMaybe x        -> TyMaybe (appsubs s x)
  | TyEither (x,y)   -> TyEither (appsubs s x, appsubs s y)
...
let rec var_in_tipo (v:int) (tp:tipo) : bool =
  ...
  (* ======= TRABALHO ======= *)
  | TyList x          -> var_in_tipo v x
  | TyMaybe x         -> var_in_tipo v x
  | TyEither (x,y)    -> (var_in_tipo v x) || (var_in_tipo v y)
...
(* coleta equações de tipo para uma expressão *)
let rec collect (g:tyenv) (e:expr) : (equacoes_tipo * tipo)  =
  ...
  | Cons (e1,e2) ->
      let (c1,tp1) = collect g e1 in
      let (c2,tp2) = collect g e2 in
      (c1@c2@[(tp2,TyList tp1)], TyList tp2)
...
let rec eval (renv:renv) (e:expr) : valor =
  ...
  | MatchLeft (e1,e2,e3) ->
      (match eval renv e1 with
        | VLeft v -> eval renv e2
        | VRight v -> eval renv e3
        | _ -> raise BugTypeInfer)
...
(* TESTES *)
(* PIPE *)
let teste_eval_pipe = eval [] (Pipe (Fn ("x", Binop (Sum, Var "x", Num 1)), Num 2))
let teste_type_infer_pipe = type_infer(Pipe (Fn ("x", Binop (Sum, Var "x", Num 1)), Num 2))

(* Nil *)
let teste_eval_nil = eval [] Nil
let teste_type_infer_nil = type_infer(Nil)

(* Cons *)
let teste_eval_cons = eval [] (Cons( Num 1, (Cons (Num 2, (Cons (Num 3, Nil))))))
let teste_type_infer_cons = type_infer(Cons( Num 1, (Cons (Num 2, (Cons (Num 3, Nil))))))

(* MatchNil *)
let teste_eval_matchnil = eval [] (MatchNil ((Cons( Num 1, (Cons (Num 2, (Cons (Num 3, Nil)))))), Num 0, Num 1))
let teste_type_infer_matchnil = type_infer(MatchNil ((Cons( Num 1, (Cons (Num 2, (Cons (Num 3, Nil)))))), Num 0, Num 1))

(* Nothing *)
let teste_eval_nothing = eval [] (Nothing)
let teste_type_infer_nothing = type_infer(Nothing)

(* Just *)
let teste_eval_just = eval [] (Just (Num 1))
let teste_type_infer_just = type_infer(Just (Num 1))

(* MatchNothing *)
let teste_eval_matchnothing = eval [] (MatchNothing (Nothing, Num 0, Num 1))
let teste_type_infer_matchnothing = type_infer(MatchNothing (Nothing, Num 0, Num 1))

(* Left *)
let teste_eval_left = eval [] (Left (Num 1))
let teste_type_infer_left = type_infer(Left (Num 1))

(* Right *)
let teste_eval_right = eval [] (Right (Num 1))
let teste_type_infer_right = type_infer(Right (Num 1))

(* MatchLeft *)
let teste_eval_matchleft = eval [] (MatchLeft (Left (Num 1), Num 0, Num 1))
let teste_type_infer_matchleft = type_infer(MatchLeft (Left (Num 1), Num 0, Num 1))

let teste_eval_matchright = eval [] (MatchLeft (Right (Num 1), Num 0, Num 1))
let teste_type_infer_matchright = type_infer(MatchLeft (Right (Num 1), Num 0, Num 1))
